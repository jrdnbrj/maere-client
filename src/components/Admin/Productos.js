...
const Productos = () => {
    ...
    return <>
        <button className="btn btn-secondary mb-3 ms-3">Agregar Producto</button>
        <section className="row" id="row-correction">
            {data && data.getProducts.map(producto => {
                return <form key={producto.name} className="product-form col-sm-12 col-lg-6" onSubmit={saveProduct}>
